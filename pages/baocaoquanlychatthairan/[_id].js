...
import { getById, updateById } from 'src/api/baocaoquanlychatthairanApi'
...
class BaoCaoQuanLyChatThaiRanEdit extends React.Component {
  static propTypes = {
  ...
  async componentDidMount() {
    this.props.setBreadCrumb(breadcrumb[slug.baocaoquanlychatthairan.edit])

    getById(this.props._id)
      .then(response => {
        const { data } = response
        this.BaoCaoQuanLyChatThaiRanForm.tranformData2Form(data)
      })
      .finally(() => {
        this.setState({
          isLoadedForm: true
    ...
    return (
      <div>
        <Spin spinning={!this.state.isLoadedForm}>
          <BaoCaoQuanLyChatThaiRanForm isEdit getRef={ref => (this.BaoCaoQuanLyChatThaiRanForm = ref)} />
...
BaoCaoQuanLyChatThaiRanEdit.Layout = DefaultLayout
export default BaoCaoQuanLyChatThaiRanEdit
