...
@Form.create()
export default class SoChuNguonThaiForm extends React.Component {
  static propTypes = {
  ...
  tranformData2Form = data => {
    let result = {
      ...
      CoQuanCap: get(data, 'CoQuanCap._id')
    }
    ...
    return (
      <Form style={{ width: '100%', minHeight: 900 }}>
        <ContainerCustomRow>
          ...
          <Row gutter={12}>
            <Col xs={24}>
              <Form.Item label='Số quyết định'>
                {getFieldDecorator('SoQuyetDinh', {
                  rules: [{ required: true, message: 'Field is required' }]
                })(<Input />)}
              </Form.Item>
            </Col>
          ...
          <Row gutter={12}>
            ...
            <Col xs={12}>
              <Form.Item label='Cơ quan cấp'>{getFieldDecorator('CoQuanCap', {})(<SelectCoQuanCapPhep />)}</Form.Item>
            </Col>
          </Row>
              ...
              <Form.Item label='Attachment'>
                {getFieldDecorator('TapTinDinhKem', {})(
                  <UploadAttachment
                    ...
                    keyUpload={ATTACHMENT_TYPE.SO_CHU_NGUON_THAI}
                  />
                )}
              </Form.Item>
            </Col>
